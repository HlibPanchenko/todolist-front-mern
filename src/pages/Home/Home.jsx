...
const Home = () => {
  ...
  const [alltasks, setAllTasks] = React.useState([]); // чтобы посчитать сколько делать страниц пагинации
  ...
  React.useEffect(() => {
    const getAllTasksForSpecificDay = async () => {
      //чтобы посчитать сколько делать страниц пагинации
      const { date, month, year } = fullDate;
      const { data } = await axios.get(
        `http://localhost:7777/get-all-tasks?page=1&limit=5000&day=${date}&month=${month}&year=${year}`
      );
      setAllTasks(data);
    };
      ...
      <div className={styles.block}>
        ...
        <div className={styles.paginationBlock}>
          {alltasks.length > 5 && (
            <Pagination
              color="primary"
              count={Math.ceil(alltasks.length / 5)}
              // count={10}
              page={page}
